...
@section('title', 'Progress Penyelesaian Rekayasa — Luar Komitmen')
...
@php
   ...
   $categoryWeight = (int) ($summary['arahan_manajemen']['count'] ?? 0)
      + (int) ($summary['rekom_insiden']['count'] ?? 0)
      + (int) ($summary['rekom_gr']['count'] ?? 0);
   ...
   $categoryStatCards = [
      [
         'key' => 'arahan_manajemen',
         'label' => 'Total Arahan Manajemen',
         'title' => 'Arahan Manajemen',
         'stat' => $summary['arahan_manajemen'],
         'items' => $arahanManajemenItems ?? [],
      ],
      [
         'key' => 'rekom_insiden',
         'label' => 'Total Rekom. Insiden',
         'title' => 'Rekom. Insiden',
         'stat' => $summary['rekom_insiden'],
         'items' => $rekomInsidenItems ?? [],
      ],
      [
         'key' => 'rekom_gr',
         'label' => 'Total Rekom Pelanggaran GR',
         'title' => 'Rekom Pelanggaran GR',
         'stat' => $summary['rekom_gr'],
         'items' => $rekomGrItems ?? [],
      ],
   ];
...
@php
   $replikasiStat = $summary['arahan_manajemen'] ?? [];
   $safetyStat = $summary['rekom_insiden'] ?? [];
   $additionalStat = $summary['rekom_gr'] ?? [];

   // Total Luar Komitmen = agregat 3 card kategori utama saja
...
<div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-4">
   <div class="crm-card crm-kpi-card crm-kpi-card--total">
      <div class="crm-kpi-head">
         <div>
            <p class="crm-kpi-label">Total Luar Komitmen</p>
            <p class="crm-kpi-subtitle">Arahan Manajemen + Rekom Insiden + Rekom GR</p>
         </div>
   ...
   @php
      ...
      $isReplikasiCard = $cardKey === 'arahan_manajemen';
      $isSafetyCard = $cardKey === 'rekom_insiden';
      $isAdditionalCard = $cardKey === 'rekom_gr';
      ...
      $cardIcon = $isReplikasiCard
         ? 'campaign'
         : ($isSafetyCard ? 'report' : ($isAdditionalCard ? 'gavel' : 'category'));
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
   ...
   @php
      $trendTheme = match ((string) ($trend['key'] ?? '')) {
         'arahan_manajemen' => 'replikasi',
         'rekom_insiden' => 'safety',
         'rekom_gr' => 'additional',
         default => 'default',
      };
   ...
   document.addEventListener('DOMContentLoaded', function () {
      ...
      function isStatusBreakdownMode(payload) {
         ...
         return meta === 'replikasi_status'
            ...
            || payload?.key === 'arahan_manajemen'
            || payload?.key === 'rekom_insiden'
