...
final class SportEvaluationEmployeeProfileUpdateRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($kodeSid !== '' && $service->isKodeSidTaken($kodeSid, $ignoreId)) {
                $validator->errors()->add('kode_sid', 'Kode SID sudah digunakan.');
            }

            $nik = trim((string) $this->input('nik', ''));
            if ($nik !== '' && $service->isNikTaken($nik, $ignoreId)) {
